/**
 * Team Management Plugin - Type Definitions
 */

export type TeamMemberStatus = 'active' | 'invited' | 'suspended' | 'deactivated';

export type DefaultRoleName = 'owner' | 'admin' | 'manager' | 'viewer';
...
export interface TeamRoleWithPermissions extends TeamRole {
...
export interface BulkSetPermissionsInput {
  roleId: string;
  permissions: Array<{
    section: string;
    subsection: string;
...
export interface TeamMemberLoginInput {
  email: string;
  password: string;
  teamId?: string;
}

export interface TeamMemberLoginResult {
  success: boolean;
  token?: string;
...
export const DEFAULT_ROLE_PERMISSIONS: Record<DefaultRoleName, Record<string, boolean>> = {
  owner: {
    '*': true,
  },
  admin: {
...
export const ADMIN_PERMISSION_SECTIONS: PermissionSection[] = [
  {
    id: 'users',
    label: 'Users',
    icon: 'Users',
