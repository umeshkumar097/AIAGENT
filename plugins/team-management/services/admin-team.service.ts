/**
 * Admin Team Service - Core admin team management for sub-admins
 */
...
export interface AdminTeamMemberWithRole extends AdminTeamMember {
  role: AdminTeamRole;
}

export class AdminTeamService {
  static async getOrCreateAdminTeam(): Promise<AdminTeam> {
  ...
  static async getAdminTeam(): Promise<AdminTeam | null> {
    const result = await db.execute(sql`SELECT * FROM admin_teams LIMIT 1`);
    if (result.rows.length === 0) return null;
    return this.mapTeamRow(result.rows[0]);
  }

  static async initializeDefaultRoles(adminTeamId: string): Promise<AdminTeamRole[]> {
    const defaultRoles = [
  ...
  static async initializeRolePermissions(roleId: string, roleName: string): Promise<void> {
  ...
  static getDefaultPermissionLevel(roleName: string): 'full' | 'manage' | 'read' | 'none' {
    switch (roleName) {
      case 'super_admin': return 'full';
      case 'admin': return 'manage';
      case 'support': return 'read';
      case 'viewer': return 'read';
      default: return 'none';
    }
  }

  static getPermissionsForLevel(level: string): { canCreate: boolean; canRead: boolean; canUpdate: boolean; canDelete: boolean } {
    switch (level) {
  ...
  static async createMember(adminTeamId: string, data: {
    email: string;
    password: string;
    firstName?: string;
    lastName?: string;
  ...
  static async getMembersByTeam(adminTeamId: string): Promise<AdminTeamMemberWithRole[]> {
    const result = await db.execute(sql`
  ...
  static async getMemberById(memberId: string): Promise<AdminTeamMember | null> {
    const result = await db.execute(sql`
  ...
  static async getMemberByEmail(email: string): Promise<AdminTeamMemberWithRole | null> {
    const result = await db.execute(sql`
      SELECT m.*, r.name as role_name, r.display_name as role_display_name, 
             r.description as role_description, r.is_system as role_is_system, r.is_default as role_is_default
      FROM admin_team_members m
  ...
  static async updateMember(memberId: string, data: {
    firstName?: string;
    lastName?: string;
    roleId?: string;
    status?: TeamMemberStatus;
    ...
    if (data.firstName !== undefined) setClauses.push(sql`first_name = ${data.firstName}`);
    if (data.lastName !== undefined) setClauses.push(sql`last_name = ${data.lastName}`);
  ...
  static async updateMemberPassword(memberId: string, newPassword: string): Promise<void> {
  ...
  static async getRolesByTeam(adminTeamId: string): Promise<AdminTeamRole[]> {
    const result = await db.execute(sql`
      SELECT * FROM admin_team_roles 
      WHERE admin_team_id = ${adminTeamId}
      ORDER BY is_system DESC, created_at ASC
    `);
    return result.rows.map((row: any) => this.mapRoleRow(row));
  }

  static async getRoleById(roleId: string): Promise<AdminTeamRole | null> {
    const result = await db.execute(sql`SELECT * FROM admin_team_roles WHERE id = ${roleId}`);
    if (result.rows.length === 0) return null;
    return this.mapRoleRow(result.rows[0]);
  }

  static async createRole(adminTeamId: string, data: {
    name: string;
    displayName: string;
    description?: string;
    copyFromRoleId?: string;
  }): Promise<AdminTeamRole> {
    const result = await db.execute(sql`
      INSERT INTO admin_team_roles (admin_team_id, name, display_name, description, is_system, is_default)
      VALUES (
        ${adminTeamId},
  ...
  static async deleteRole(roleId: string): Promise<void> {
    ...
    if (parseInt((membersWithRole.rows[0] as any).count) > 0) {
  ...
  static async getPermissionsForRole(roleId: string): Promise<AdminTeamPermission[]> {
    const result = await db.execute(sql`
  ...
  static async bulkSetPermissions(roleId: string, permissions: Array<{
    section: string;
    subsection: string;
    canCreate: boolean;
    canRead: boolean;
    ...
    const CHUNK_SIZE = 50;
    const chunks: typeof permissions[] = [];
    ...
    await Promise.all(chunks.map(async (chunk) => {
      const values = chunk.map(perm => 
        sql`(${roleId}, ${perm.section}, ${perm.subsection}, ${perm.canCreate}, ${perm.canRead}, ${perm.canUpdate}, ${perm.canDelete})`
      );
  ...
  static async getMemberCount(adminTeamId: string): Promise<number> {
    const result = await db.execute(sql`
  ...
  static async validateSession(token: string): Promise<{
    ...
    try {
      ...
      // For admin team members, the adminId is the team_id (admin team is a platform-level team)
      return {
        memberId: session.member_id,
        teamId: session.admin_team_id,
        adminId: session.admin_team_id, // Use team ID for admin context
  ...
  private static mapTeamRow(row: any): AdminTeam {
    return {
      id: row.id,
      name: row.name,
  ...
  private static mapMemberRow(row: any): AdminTeamMember {
    return {
      id: row.id,
      adminTeamId: row.admin_team_id,
      email: row.email,
  ...
  private static mapRoleRow(row: any): AdminTeamRole {
    return {
      id: row.id,
      adminTeamId: row.admin_team_id,
      name: row.name,
  ...
  static async getActivityLogs(
    adminTeamId: string,
    options?: { limit?: number; offset?: number; memberId?: string; action?: string; search?: string }
    ...
    return {
      logs: result.rows.map((row: any) => ({
        ...
        memberName: row.member_first_name && row.member_last_name 
