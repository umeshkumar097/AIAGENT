/**
 * Team Service - Core team management business logic
 */
...
export class TeamService {
  static async createTeam(userId: string | number, name?: string): Promise<Team> {
  ...
  static async getTeamByUserId(userId: string | number): Promise<Team | null> {
    const result = await db.execute(sql`
  ...
  static async initializeDefaultRoles(teamId: string): Promise<TeamRole[]> {
    const defaultRoles: Array<{ name: DefaultRoleName; displayName: string; description: string; isDefault: boolean }> = [
  ...
  static async initializeRolePermissions(roleId: string, roleName: DefaultRoleName): Promise<void> {
  ...
  static async createMember(teamId: string, data: CreateTeamMemberInput): Promise<TeamMember> {
  ...
  static async getMemberByEmail(teamId: string, email: string): Promise<TeamMember | null> {
    const result = await db.execute(sql`
  ...
  static async getMemberWithRole(memberId: string): Promise<TeamMemberWithRole | null> {
    const result = await db.execute(sql`
  ...
  static async getMembersByTeam(teamId: string): Promise<TeamMemberWithRole[]> {
    const result = await db.execute(sql`
      SELECT m.*, r.name as role_name, r.display_name as role_display_name, 
             r.description as role_description, r.is_system as role_is_system, r.is_default as role_is_default
      FROM team_members m
  ...
  static async updateMember(memberId: string, data: UpdateTeamMemberInput): Promise<TeamMember> {
    const setClauses: ReturnType<typeof sql>[] = [];
  ...
  static async getRolesByTeam(teamId: string): Promise<TeamRole[]> {
    const result = await db.execute(sql`
  ...
  static async getRoleById(roleId: string): Promise<TeamRole | null> {
    const result = await db.execute(sql`
  ...
  static async createRole(teamId: string, data: CreateTeamRoleInput): Promise<TeamRole> {
    const result = await db.execute(sql`
      INSERT INTO team_roles (team_id, name, display_name, description, is_system, is_default)
      VALUES (
        ${teamId}::uuid, 
  ...
  static async updateRole(roleId: string, data: UpdateTeamRoleInput): Promise<TeamRole> {
  ...
  private static mapRoleRow(row: any): TeamRole {
    return {
      id: row.id,
      teamId: row.team_id,
      name: row.name,
