...
export class EmailTemplateService {
  ...
  async create(
    userId: string,
    data: { name: string; subject: string; htmlBody: string; variables?: string[]; designJson?: any }
  ...
  async update(
    userId: string,
    id: string,
    data: { name?: string; subject?: string; htmlBody?: string; variables?: string[]; isActive?: boolean; designJson?: any }
    ...
    const pgArray = `{${variables.map(v => `"${v.replace(/"/g, '\\"')}"`).join(',')}}`;
    const designJson = data.designJson !== undefined
      ? (data.designJson ? JSON.stringify(data.designJson) : null)
      : (existing.designJson ? JSON.stringify(existing.designJson) : null);
...
<body>
  ...
  private buildUnlayerRow(
    rowN: number,
    colN: number,
    contents: Array<{ n: number; html: string; padding?: string }>,
    bgColor: string = '#ffffff',
    rowPadding: string = '0px'
  ): any {
    return {
      id: `u_row_${rowN}`,
      cells: [1],
  ...
  private buildUnlayerDesign(rows: any[], totalTexts: number): any {
    return {
      counters: {
  ...
  private getDefaultDesigns(): Record<string, any> {
    ...
    const headerHtml = (title: string) =>
      ...
      `<h1 style="margin:0 0 4px 0;font-size:26px;font-weight:700;color:#ffffff;letter-spacing:-0.5px;">{{company_name}}</h1>` +
      `<p style="margin:0;font-size:13px;color:#94a3b8;">${title}</p>` +
      `</div>`;

    const footerHtml = () =>
      `<p style="margin:0;font-size:12px;color:#94a3b8;text-align:center;">&copy; {{company_name}}. All rights reserved.</p>`;

    const detailsTableHtml = (rows: string[][]) => {
      const trs = rows.map(([label, value]) =>
        `<tr><td style="padding:10px 0;border-bottom:1px solid #e5e7eb;color:#6b7280;font-size:14px;width:40%;">${label}</td>` +
        `<td style="padding:10px 0;border-bottom:1px solid #e5e7eb;text-align:right;font-weight:500;color:#1f2937;font-size:14px;">${value}</td></tr>`
      ).join('');
      return `<table style="width:100%;border-collapse:collapse;">${trs}</table>`;
    };

    const highlightBox = (content: string) =>
      `<div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;padding:20px 24px;">${content}</div>`;

    const alertBox = (content: string, color: string = '#f59e0b', bg: string = '#fef3c7') =>
      `<div style="background:${bg};border-radius:6px;padding:14px 18px;">` +
      `<span style="font-size:14px;color:#1f2937;">${content}</span></div>`;

    const p = (text: string, style: string = '') =>
      `<p style="margin:0;font-size:15px;line-height:1.6;color:#374151;${style}">${text}</p>`;

    const h2 = (text: string) =>
      `<h2 style="margin:0 0 12px 0;font-size:22px;font-weight:600;color:#1e293b;">${text}</h2>`;
    ...
    return {
      'Appointment Confirmation': (() => {
        const rows = [
          ...
          this.buildUnlayerRow(2, 2, [{
            n: 2,
            html: h2('Appointment Confirmed') + gap() +
              p('Hi {{contact_name}},') + gap() +
              p('Your appointment has been successfully booked. Here are the details:'),
          ...
          this.buildUnlayerRow(4, 4, [{
            n: 4,
            html: alertBox('<strong>You\'re all set!</strong> We look forward to seeing you.', '#22c55e', '#dcfce7') +
              gap() + p('{{notes}}') + gap() +
              p('If you need to reschedule or cancel, please contact us.', 'font-size:13px;color:#6b7280;'),
      ...
      'Appointment Reminder': (() => {
        const rows = [
          ...
          this.buildUnlayerRow(2, 2, [{
            n: 2,
            html: h2('Appointment Reminder') + gap() +
              p('Hi {{contact_name}},') + gap() +
          ...
          this.buildUnlayerRow(4, 4, [{
            n: 4,
            html: alertBox('<strong>Please arrive on time.</strong> If you need to reschedule, contact us as soon as possible.', '#3b82f6', '#dbeafe') +
              gap() + p('{{notes}}') + gap() +
              p('We look forward to seeing you!', 'font-size:13px;color:#6b7280;'),
            padding: '0px 40px 32px',
          }], BODY_BG),
          this.buildUnlayerRow(5, 5, [{ n: 5, html: footerHtml(), padding: '20px 40px' }], FOOTER_BG),
        ];
      ...
      'Call Follow-Up': (() => {
        const rows = [
          ...
          this.buildUnlayerRow(2, 2, [{
            n: 2,
            html: h2('Thank You for Your Call') + gap() +
              p('Hi {{contact_name}},') + gap() +
              p('Thank you for speaking with us today. We appreciate your time and wanted to follow up with a summary of our conversation.'),
            padding: '32px 40px 20px',
          }], BODY_BG),
          this.buildUnlayerRow(3, 3, [{
            n: 3,
            html: highlightBox(
              `<p style="margin:0 0 10px 0;font-weight:600;font-size:14px;color:#1e293b;">Call Summary</p>` +
              `<p style="margin:0;font-size:14px;color:#4b5563;line-height:1.6;">{{call_summary}}</p>`
            ),
            padding: '0px 40px 20px',
          }], BODY_BG),
          this.buildUnlayerRow(4, 4, [{
            n: 4,
            html: p('{{next_steps}}') + gap() +
              p('If you have any questions or need further assistance, please don\'t hesitate to reach out.', 'font-size:13px;color:#6b7280;'),
            padding: '0px 40px 32px',
          }], BODY_BG),
          this.buildUnlayerRow(5, 5, [{ n: 5, html: footerHtml(), padding: '20px 40px' }], FOOTER_BG),
        ];
      ...
      'Missed Call': (() => {
        const rows = [
          ...
          this.buildUnlayerRow(2, 2, [{
            n: 2,
            html: h2('We Missed Your Call') + gap() +
              p('Hi {{contact_name}},') + gap() +
              p('We noticed we weren\'t able to connect with you during our recent call attempt. We\'d love to speak with you at a time that works better.'),
            padding: '32px 40px 20px',
          }], BODY_BG),
          this.buildUnlayerRow(3, 3, [{
            n: 3,
            html: alertBox('<strong>We tried reaching you</strong> but were unable to connect. Please feel free to call us back or let us know a convenient time.') +
              gap() + p('{{message}}') + gap() +
              p('We look forward to connecting with you soon.', 'font-size:13px;color:#6b7280;'),
            padding: '0px 40px 32px',
          }], BODY_BG),
      ...
      'Welcome / Inquiry Response': (() => {
        const rows = [
          ...
          this.buildUnlayerRow(2, 2, [{
            n: 2,
            html: h2('Thank You for Your Inquiry') + gap() +
              p('Hi {{contact_name}},') + gap() +
              p('Thank you for reaching out to us. We\'ve received your inquiry and wanted to provide you with some helpful information.'),
            padding: '32px 40px 20px',
          }], BODY_BG),
          this.buildUnlayerRow(3, 3, [{
            n: 3,
          ...
          this.buildUnlayerRow(4, 4, [{
            n: 4,
            html: alertBox('<strong>We\'re here to help!</strong> A member of our team will follow up with you shortly if needed.', '#22c55e', '#dcfce7') +
              gap() + p('{{additional_info}}') + gap() +
              p('If you have any further questions, don\'t hesitate to contact us.', 'font-size:13px;color:#6b7280;'),
            padding: '0px 40px 32px',
          }], BODY_BG),
          this.buildUnlayerRow(5, 5, [{ n: 5, html: footerHtml(), padding: '20px 40px' }], FOOTER_BG),
        ];
  ...
  async seedDefaultTemplates(userId: string): Promise<void> {
    ...
    const toRepair = defaults.filter(d => {
      ...
      const htmlHasMeaningfulContent = expectedVars.length === 0
        ? (!!t.htmlBody && t.htmlBody.includes('email-body'))
        : expectedVars.some(v => t.htmlBody && t.htmlBody.includes(`{{${v}}}`));
    ...
    if (toRepair.length > 0) {
      ...
      for (const tmpl of toRepair) {
        try {
          const existing = existingMap.get(tmpl.name)!;
          const design = defaultDesigns[tmpl.name];
