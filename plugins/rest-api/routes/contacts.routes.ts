/**
 * ============================================================
 * REST API Plugin - Contacts/Leads Routes
 * Endpoints for managing CRM leads (contacts)
 * Uses the leads table which has userId for ownership
 * ============================================================
 */

import { Router, Response } from 'express';
import { apiAuthMiddleware, asyncHandler } from '../middleware/auth.middleware.js';
import type { AuthenticatedApiRequest, ApiResponse, BulkImportContactsResponse } from '../types.js';
...
/**
 * GET /v1/contacts - List CRM leads/contacts
 */
router.get(
  '/',
...
/**
 * POST /v1/contacts - Create a CRM lead/contact
 */
router.post(
  '/',
  ...
  asyncHandler(async (req: AuthenticatedApiRequest, res: Response) => {
    ...
    const [existing] = await db
      .select()
      .from(leads)
      .where(and(eq(leads.userId, userId), eq(leads.phone, data.phone)))
      .limit(1);
    ...
    const [contact] = await db
      ...
      .values({
        ...
        sourceType: 'api', // Created via API
        stage: 'new',
      })
      .returning();
...
/**
 * POST /v1/contacts/bulk-import - Bulk import contacts
 */
router.post(
  '/bulk-import',
  apiAuthMiddleware('contacts:write'),
  asyncHandler(async (req: AuthenticatedApiRequest, res: Response) => {
    ...
    for (let i = 0; i < newContacts.length; i++) {
      ...
      try {
        ...
        const [existing] = await db
          .select()
          .from(leads)
          .where(and(eq(leads.userId, userId), eq(leads.phone, contact.phone)))
          .limit(1);
    ...
    const responseData: BulkImportContactsResponse = {
      imported,
      skipped,
      errors: errors.map(e => ({ row: e.row, phoneNumber: e.phone, error: e.error })),
    };
    
    const response: ApiResponse<BulkImportContactsResponse> = {
      success: true,
      data: responseData,
      meta: { requestId: req.requestId, timestamp: new Date().toISOString() },
    };
