...
export const scheduledCallbacks = pgTable("scheduled_callbacks", {
  ...
  variables: jsonb("variables").$type<Record<string, string>>(), // {{key}} substitutions for prompt + first message
  source: text("source").notNull().default("agent"), // agent | manual | api
  externalRef: text("external_ref"), // caller's idempotency key / their record id, unique per user
  createdAt: timestamp("created_at").notNull().defaultNow(),
  updatedAt: timestamp("updated_at").notNull().defaultNow(),
}, (table) => ({
