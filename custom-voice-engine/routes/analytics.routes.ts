/**
 * Analytics Routes — Voice engine specific metrics
 */
...
export function createAnalyticsRouter(): Router {
    ...
    try {
      ...
      res.json({
        success: true, data: {
          calls: calls.rows[0],
          usage: usage.rows[0],
          activeCalls: (active.rows[0] as any)?.count || 0,
        }
      });
    } catch (err: any) {
  ...
  router.get('/calls-per-day', async (req: Request, res: Response) => {
    try {
      const userId = (req as any).userId;
      const days = parseInt(req.query.days as string) || 30;
  ...
  router.get('/provider-latency', async (req: Request, res: Response) => {
    try {
      const userId = (req as any).userId;
