 ...
 *
 * Admin panel routes for managing global call recording storage
 * provider configurations (Local, AWS S3, GCS, DigitalOcean, Wasabi).
 * ============================================================
 */
...
export function createAdminStorageRouter(): Router {
  ...
  router.get('/', async (_req: Request, res: Response) => {
    try {
      ...
      const results = await db
        .select()
        .from(globalSettings)
        .where(inArray(globalSettings.key, keys));
  ...
  router.post('/', async (req: Request, res: Response) => {
    try {
      ...
      // 1. Fetch existing settings to preserve masked keys
      const keys = Object.values(STORAGE_KEYS);
      ...
      updates.push({
        ...
        value: retentionDays !== undefined ? Number(retentionDays) : 30,
      ...
      // Preserve masked secret key
      const resolvedSecretKey = isMasked(secretKey)
        ? existingMap[STORAGE_KEYS.secretKey] || ''
        : secretKey || '';
      updates.push({
      ...
      // Preserve GCS credentials if masked
      const resolvedGcsCredentials = isMasked(gcsCredentials)
        ? existingMap[STORAGE_KEYS.gcsCredentials] || ''
        : gcsCredentials || '';
      updates.push({
  ...
  router.post('/activate', async (req, res) => {
    ...
    await db.insert(globalSettings)
      .values({ key: 've_storage_active_provider', value: provider, description: 'Active storage provider' })
      .onConflictDoUpdate({ target: globalSettings.key, set: { value: provider } });
  ...
  router.post('/test', async (req: Request, res: Response) => {
    try {
      ...
      // Retrieve existing credentials if they are masked in the request
      ...
      if (['s3', 'do_spaces', 'wasabi'].includes(provider)) {
        ...
        // Attempt a live connection test using @aws-sdk/client-s3 dynamically
        try {
          // @ts-ignore
          ...
          // Fallback if SDK is not installed or import fails
          if (s3Err.code === 'MODULE_NOT_FOUND' || s3Err.message?.includes('Cannot find module')) {
            ...
            // Check host resolution or basic format validation
            const host = endpoint 
      ...
      if (provider === 'gcs') {
        ...
        // Validate JSON format of service account credentials
        try {
        ...
        // Attempt a live connection check dynamically using @google-cloud/storage
        try {
          // @ts-ignore
